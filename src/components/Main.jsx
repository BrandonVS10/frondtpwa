...
function Main() {
  ...
  useEffect(() => {
    if (userRole === "admin") {
      fetch("https://back00pwa.onrender.com/auth/users")
        .then((response) => {
  ...
  const registerServiceWorker = async () => {
    try {
      ...
      const response = await fetch("https://back00pwa.onrender.com/auth/suscripcion", {
        method: "POST",
  ...
  const handleSendMessage = async (user) => {
    try {
      ...
      const response = await fetch("https://back00pwa.onrender.com/auth/suscripcionMod", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
